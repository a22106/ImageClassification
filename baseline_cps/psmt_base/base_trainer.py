import json
import torch
from psmt_utils import helpers
import psmt_utils.lr_scheduler as lr_scheduler
from torch.nn.parallel import DistributedDataParallel as DDP
import torch.distributed as dist
from itertools import chain
from psmt_utils.helpers import group_weight, init_weight
#from psmt_utils.download_to_pvc import *
import os
...
class BaseTrainer:
    def __init__(self, model, config, iters_per_epoch, train_logger=None, args=None):
        ...
        if self.args.ddp:
            ...
            # SETTING THE DEVICE
            self.device, availble_gpus = self._get_available_devices(0)
        ...
        self.lr_scheduler_s = getattr(lr_scheduler, "Poly")(optimizer=self.optimizer_s,
                                                                            num_epochs=self.epochs,
                                                                            iters_per_epoch=iters_per_epoch)

        self.warm_up_epoch = config['MODEL']['psmt']['warm_up_epoch']
        # MONITORING
        self.mnt_current = .0
