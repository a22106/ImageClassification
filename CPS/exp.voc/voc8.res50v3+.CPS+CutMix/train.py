...
with Engine(custom_parser=parser) as engine:
    ...
    train_l_loader, train_u_loader0, valid_l_loader, _ = data_loader.build(supervised=False, collate_fn=mask_collate_fn)
    _, train_u_loader1, _, _ = data_loader.build(supervised=False, collate_fn=collate_fn)
    ...
    # !Wandb
    if config.logger_wandb == True: ## 사용시 본인 wandb 계정 입력
        wandb_project_serial = 'CPS_Cutmix'
    ...
    # unsupervised_train_loader -> train_u_loader
    engine.register_state(dataloader=train_l_loader, model=model,
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        dataloader = iter(train_l_loader)
        unsupervised_dataloader_0 = iter(train_u_loader0)
        unsupervised_dataloader_1 = iter(train_u_loader1)
        ...
        for idx in pbar:
            ...
            # Set Recorder row
            row_dict = dict()
            row_dict['epoch_index'] = idx
            row_dict['train_serial'] = train_serial

            row_dict['train_loss'] = loss.item()
            ...
            end_time = time.time()
            row_dict['train_elapsed_time'] = end_time - start_time
            wandb.log(row_dict)
